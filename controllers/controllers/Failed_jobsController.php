 ...
 * @category  Controller
 */
class Failed_jobsController extends BaseController{
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("failed_jobs.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Failed Jobs";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("failed_jobs/view.php", $record);
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","uuid","connection","queue","payload","exception");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'uuid' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("failed_jobs.id", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Failed Jobs";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("failed_jobs/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("failed_jobs.id", $arr_rec_id, "in");
